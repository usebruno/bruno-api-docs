...
import { toBool } from '@usebruno/common/utils';

// inlined from @usebruno/common: not on its browser-safe utils entry
const toMaxRedirects = (value: unknown): number =>
  typeof value === 'number' && Number.isFinite(value) && value >= 0 ? Math.trunc(value) : 5;
import type {
  HttpRequest,
  HttpRequestSettings,
  HttpRequestExample,
